...
**Source of Truth** `Current Task` 的身份、状态与执行资格以 `.context/current-task-metadata.yaml` 为单一事实源。`.context/current-task.md` 中的进度标记、说明、人工验收文案（如有）都只是本地工作副本；`.context/current-phase.md` 仅用于读取 Phase 上下文，其中的说明、人工验收文案都只是本地工作副本。未上传、未落库的信息不构成有效状态，也不能覆盖 Metadata 的定义。
...
- Step 4: 检查确认所有步骤条目已按预期完成并标注完成状态；完成后仅表示“本地执行已收敛，已具备验收条件”，不表示后端状态已更新。
